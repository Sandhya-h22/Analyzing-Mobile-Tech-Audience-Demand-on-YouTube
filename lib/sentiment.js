import { inferSentiments } from "./mlClient.js";
...
function scoreToMagnitude(score) {
  if (score > 1.5 || score < -1.5) return "strong";
  if (score > 0.3 || score < -0.3) return "mild";
  return "neutral";
}
...
export function analyseSentiment(comment) {
  ...
  return { sentiment: label, magnitude: scoreToMagnitude(score), sentimentScore: parseFloat(score.toFixed(2)) };
}

function normalizeSentimentInference(inference = {}) {
  const label = inference.label || "neutral";
  const score = Number(inference.score || 0);
  return {
    sentiment: label,
    magnitude: inference.magnitude || scoreToMagnitude(score),
    sentimentScore: parseFloat(score.toFixed(4)),
    sentimentModel: inference.model || null,
    sentimentProbabilities: inference.probabilities || null,
  };
}
...
export function summarizeIntents(results) {
  const intentCounts = {};
  ...
  return Object.values(intentCounts).sort((a, b) => b.count - a.count);
}

export function summarizeSentiments(results) {
  return {
    ...
    avgScore: parseFloat((results.reduce((sum, comment) => sum + Number(comment.sentimentScore || 0), 0) / Math.max(1, results.length)).toFixed(2)),
  };
}

function buildPhoneTopics(comments = []) {
  const grouped = comments.reduce((acc, comment) => {
    const key = comment.subtopic || "general_tech";
    if (!acc[key]) acc[key] = [];
  ...
  return Object.entries(grouped)
    .map(([key, items]) => ({
      topicKey: key,
      label: key.replace(/^smartphone_/, "").replace(/_/g, " ").replace(/\b\w/g, (char) => char.toUpperCase()),
      emoji: "Phone",
      color: "#00d4ff",
      commentCount: items.length,
      comments: items,
      weightedScore: items.length / Math.max(1, comments.length),
    }))
    .sort((a, b) => b.commentCount - a.commentCount)
    .slice(0, 4);
}

function citePhoneExamples(comments = [], limit = Infinity) {
  return [...comments]
    .sort((a, b) => ((b.likeCount || 0) + (b.replyCount || 0)) - ((a.likeCount || 0) + (a.replyCount || 0)))
    .slice(0, limit)
    .map((comment) => ({
      ...
      author: comment.author,
      text: comment.text || comment.original || "",
      likeCount: comment.likeCount || 0,
      replyCount: comment.replyCount || 0,
      publishedAt: comment.publishedAt,
      sentiment: comment.sentiment || "neutral",
      demandScore: comment.demandScore || 0,
      demandMatches: comment.demandMatches || [],
      isDemand: Boolean(comment.isDemand),
      subtopic: comment.subtopic || null,
      primaryIntent: comment.primaryIntent || "general",
      videoTitle: comment.videoTitle,
      videoChannel: comment.videoChannel,
      videoThumbnail: comment.videoThumbnail,
    }));
}

export function buildPhoneMentions(phoneBuckets = {}, metadata = {}) {
  return Object.values(phoneBuckets)
    .map((bucket) => {
      const comments = bucket.comments || [];
      const sentiment = summarizeSentiments(comments);
      const intentSummary = summarizeIntents(comments);
      const suggestions = extractContentSuggestions(comments.filter((comment) => comment.isTech));
      ...
      const topics = buildPhoneTopics(demandComments.length ? demandComments : comments);
      const viralityScore = computeViralityScore(metadata, comments, sentiment, {
        total: comments.length,
        demand: demandComments.length,
      });
      const demands = generateActionableSteps(
        topics,
        sentiment,
        intentSummary,
        suggestions,
      ...
      return {
        ...
        brand: bucket.brand,
        mentionCount: comments.length,
        sentiment,
        intentSummary,
        demands,
        demandCount: demandComments.length,
        demandPercent: Math.round((demandComments.length / Math.max(1, comments.length)) * 100),
        exampleComments: citePhoneExamples(comments),
      };
    })
...
export function processAllSentiments(comments) {
  ...
  return {
    comments: results,
    stats: summarizeSentiments(results),
    intentSummary: summarizeIntents(results),
  };
}

export function buildSentimentTimeline(comments = [], bucket = "day") {
  const buckets = new Map();

  for (const comment of comments) {
    const date = new Date(comment.publishedAt || 0);
    ...
    const label = comment.sentiment || "neutral";
    entry[label] = (entry[label] || 0) + 1;
    entry.total += 1;
    entry.avgScore += Number(comment.sentimentScore || 0);
  ...
  return [...buckets.values()]
    ...
    .map((entry) => ({
      ...entry,
      avgScore: parseFloat((entry.avgScore / Math.max(1, entry.total)).toFixed(3)),
      positiveRate: Math.round((entry.positive / Math.max(1, entry.total)) * 100),
      negativeRate: Math.round((entry.negative / Math.max(1, entry.total)) * 100),
    }));
}

export async function processAllSentimentsWithML(comments) {
  if (!comments?.length) {
    return {
      comments: [],
      stats: { positive: 0, negative: 0, neutral: 0, total: 0, avgScore: 0 },
      intentSummary: [],
      engine: { mode: "rule-based", stage: "sentiment", provider: "local-heuristics", model: null },
    };
  }

  try {
    const response = await inferSentiments(comments.map((comment) => comment.text || comment.original || ""));
    const results = Array.isArray(response?.results) ? response.results : [];
    if (results.length !== comments.length) {
      throw new Error("Sentiment result count mismatch");
    }

    const merged = comments.map((comment, index) => ({
      ...comment,
      ...normalizeSentimentInference(results[index]),
      ...(comment.intents ? {} : detectIntent(comment)),
    }));

    return {
      comments: merged,
      stats: summarizeSentiments(merged),
      intentSummary: summarizeIntents(merged),
      engine: {
        mode: "ml",
        stage: "sentiment",
        provider: "ml-service",
        model: merged.find((comment) => comment.sentimentModel)?.sentimentModel || null,
      },
    };
  } catch {
    if (STRICT_ML_MODE) {
      throw new Error("ML sentiment inference is required but unavailable. Start the ML service before running analysis.");
    }
    const fallback = processAllSentiments(comments);
    return {
      ...fallback,
      engine: { mode: "rule-based", stage: "sentiment", provider: "local-heuristics", model: null },
    };
