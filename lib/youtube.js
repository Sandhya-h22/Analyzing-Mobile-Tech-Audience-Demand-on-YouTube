...
function normalizeCommentSnippet(snippet, { commentId, videoId, parentId = null, replyCount = 0 }) {
  return {
    commentId,
    parentId,
...
async function fetchCommentReplies(parentId, videoId, KEY, afterDate = null, remaining = Infinity) {
  const replies = [];
  let pageToken = null;
  const cutoff = afterDate ? new Date(afterDate).getTime() : null;
  const unlimited = !Number.isFinite(remaining);

  while (unlimited || replies.length < remaining) {
    const pageSize = Math.min(unlimited ? 100 : Math.max(0, remaining - replies.length), 100);
    if (pageSize <= 0) break;

    let url = `${BASE}/comments?part=snippet&parentId=${parentId}&maxResults=${pageSize}&key=${KEY}`;
    if (pageToken) url += `&pageToken=${pageToken}`;

    const res = await fetch(url);
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      throw new Error(err.error?.message || `YouTube replies error: ${res.status}`);
    }
    ...
    for (const item of items) {
      ...
      const ts = new Date(snippet.publishedAt).getTime();
      if (cutoff && ts < cutoff) continue;
      replies.push(normalizeCommentSnippet(snippet, {
        commentId: item.id,
        videoId,
        parentId,
      }));
      if (!unlimited && replies.length >= remaining) break;
    }
...
export async function fetchComments(videoId, maxComments = Infinity, KEY, afterDate = null, options = {}) {
  const comments  = [];
  let pageToken   = null;
  const cutoff    = afterDate ? new Date(afterDate).getTime() : null;
  const unlimited = !Number.isFinite(maxComments);
  const includeReplies = options.includeReplies !== false;
  const order = options.order || (unlimited ? "time" : "relevance");

  while (unlimited || comments.length < maxComments) {
    const remaining = unlimited ? 100 : Math.max(0, maxComments - comments.length);
    const pageSize = Math.min(remaining, 100);
    if (pageSize <= 0) break;
    let url = `${BASE}/commentThreads?part=snippet&videoId=${videoId}&maxResults=${pageSize}&order=${order}&key=${KEY}`;
    if (pageToken) url += `&pageToken=${pageToken}`;
    const res  = await fetch(url);
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
    ...
    for (const item of items) {
      ...
      const parentId = item.snippet.topLevelComment.id || item.id;
      comments.push(normalizeCommentSnippet(top, {
        commentId: parentId,
        videoId,
        replyCount: item.snippet.totalReplyCount || 0,
      }));

      if (includeReplies && item.snippet.totalReplyCount > 0 && (unlimited || comments.length < maxComments)) {
        const replyLimit = unlimited ? Infinity : maxComments - comments.length;
        const replies = await fetchCommentReplies(parentId, videoId, KEY, afterDate, replyLimit);
...
export async function resolveChannelId(channelInput, KEY) {
  ...
  const handleRes = await fetch(`${BASE}/channels?part=id&forHandle=${encodeURIComponent(handle)}&key=${KEY}`);
  if (handleRes.ok) {
    const handleData = await handleRes.json();
    const directChannelId = handleData.items?.[0]?.id;
...
export async function fetchChannelInfo(channelId, KEY) {
  ...
  return {
    ...
    subscriberCount: item.statistics?.hiddenSubscriberCount ? null : parseStat(item.statistics?.subscriberCount),
...
export async function fetchRecentVideos(channelId, daysBack = 7, maxResults = 10, KEY) {
  ...
  while (recentItems.length < maxResults) {
    let playlistUrl = `${BASE}/playlistItems?part=snippet&playlistId=${uploadsId}&maxResults=50&key=${KEY}`;
    ...
    for (const item of items) {
      ...
      const isRecent = publishedAt && new Date(publishedAt).getTime() >= cutoff;
      if (isRecent) recentItems.push(item);
      if (recentItems.length >= maxResults) break;
    }

    const oldestItem = items[items.length - 1];
    const oldestPublishedAt = oldestItem?.snippet?.publishedAt;
    const reachedOlderVideos = oldestPublishedAt && new Date(oldestPublishedAt).getTime() < cutoff;
    pageToken = playlistData.nextPageToken;
    if (!pageToken || reachedOlderVideos || items.length === 0) break;
  }

  const limitedRecentItems = recentItems.slice(0, maxResults);

  const videoIds = limitedRecentItems
...
export async function fetchVideoData(url, maxComments = Infinity, KEY, commentOptions = {}) {
  ...
  const [metadata, comments] = await Promise.all([
    ...
    fetchComments(videoId, maxComments, key, null, commentOptions),
  ]);
