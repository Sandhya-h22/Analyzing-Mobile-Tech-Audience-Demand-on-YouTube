...
export default function Navbar() {
  ...
  const links = [
    ...
    { href: "/history", label: `🕐 History${historyCount > 0 ? ` (${historyCount})` : ""}` },
      ...
      <div className="container" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", height: 56 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 2 }}>
          {links.map(({ href, label }) => (
            <Link key={href} href={href} style={{ padding: "6px 12px", borderRadius: "var(--radius)", fontSize: 12, fontWeight: 700, color: router.pathname === href ? "var(--accent)" : "var(--text-muted)", background: router.pathname === href ? "#00d4ff11" : "transparent", textDecoration: "none", transition: "all 0.2s", letterSpacing: "0.02em", whiteSpace: "nowrap" }}>
              {label}
            </Link>
          ))}
        </div>
